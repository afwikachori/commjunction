...
<!-- MODAL FIND FRIEND -->
<div class="modal fade" id="find_friend_modal" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="background-color: #ffffff;">
<form method="POST" id="form_send_message" action="{{route('friend_send_message')}}">
{{ csrf_field() }}
                ...
                $.each(result, function (i, item) {
                  ...
                  // $date=item.author_name;
                    ...
                    newslist += '<div class="news-card" id="' + item.id + '">' +
                        ...
                        '<img src="' + server_cdn + item.url + '" class="img-fluid"' +
                        ...
                        '<a href="/subscriber/detail_news/'+ item.id +'" class="btn btn-tosca btn-sm konco">' +
                        'See Detail' +
                        '</a></div>' +
                        '</div>' +
                        '</div>';
